...
const LoginPage = () => {
  ...
  useEffect(() => {
    // Check if user is signed in and has a role
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-gray-100">
      <SignIn.Root>
        <SignIn.Step
          name="start"
          className="relative isolate w-full space-y-8 rounded-2xl bg-white px-4 py-10 shadow-md ring-1 ring-inset ring-gray-100 before:absolute before:inset-0 before:-z-10 before:rounded-2xl before:bg-white/50 sm:w-96 sm:px-8"
        >
          <header className="text-center">
            ...
            <h1 className="mt-4 text-xl font-medium tracking-tight">
              Sign in to SchoolApp
            </h1>
          </header>
          <Clerk.GlobalError className="block text-sm text-rose-400" />
          <Clerk.Field name="identifier" className="group/field relative">
            <Clerk.Label className="absolute left-2 top-0 -translate-y-1/2 bg-white px-2 font-mono text-xs/4 text-black before:absolute before:inset-0 before:-z-10 before:bg-white/50 group-focus-within/field:text-gray-500 group-data-[invalid]/field:text-rose-400">
              Email address
            </Clerk.Label>
            <Clerk.Input
              type="text"
              required
              className="w-full rounded-lg bg-transparent px-4 py-2.5 text-sm text-black outline-none ring-1 ring-inset ring-black/20 hover:ring-black/30 focus:shadow-[0_0_6px_0] focus:shadow-emerald-500/20 focus:ring-[1.5px] focus:ring-gray-800 data-[invalid]:shadow-rose-400/20 data-[invalid]:ring-rose-400"
            />
          ...
          <SignIn.Action
            submit
            className="w-full rounded-md bg-zinc-950 px-3.5 py-1.5 text-center text-sm font-medium text-white shadow outline-none ring-1 ring-inset ring-zinc-950 hover:bg-zinc-800 focus-visible:outline-[1.5px] focus-visible:outline-offset-2 focus-visible:outline-zinc-950 active:text-white/70 cursor-pointer"
          >
            Sign In
          </SignIn.Action>
